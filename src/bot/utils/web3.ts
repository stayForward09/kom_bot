...
export const getETHBalance = async (address: string, chainId: number, provider?: any) => {
    try {
        if (!provider) {
            ...
            // web3 provider
            provider = new ethers.JsonRpcProvider(_chain.rpc)
        }
        const _balance = await provider.getBalance(address)
        return Number(formatEther(_balance))
    } catch (err) {
        return 0
    }
}
...
export const getKOMBalance = async (chainId: number, address: string, provider?: any) => {
    try {
        ...
        return Number(formatUnits(_balance, 8))
    } catch (err) {
        return 0.0
    }
}
...
export const getKOMVBalance = async (chainId: number, address: string, provider?: any) => {
    try {
        ...
        const contract = new ethers.Contract(CONTRACT_ADDRESS, abi, provider)
        const _balance = await contract.balanceOf(address)
        return Number(formatUnits(_balance, 0))
    } catch (err) {
        return 0.0
    }
}
...
export const getTokenBalances = async (chainId: number, address: string) => {
    try {
        ...
        // get balances
        const [nativeBalance, komBalance, komvBalance, komTokenPrice] = await Promise.all([getETHBalance(address, chainId, provider), getKOMBalance(chainId, address, provider), getKOMVBalance(chainId, address, provider), getKOMTokenPrice()])
        return {
            nativeBalance,
            komBalance,
            komvBalance,
...
export const getStakingV2Details = async (chainId: number, address: string) => {
    try {
        ...
        return Number(formatUnits(_amount, 8))
    } catch (err) {
        return 0.0
    }
}
...
export const getStakingV3Detail = async (chainId: number, address: string, index: number, provider?: any) => {
    ...
    const _contractStaking = new ethers.Contract(STAKING_CONTRACT_ADDRESS, STAKING_ABI, provider)
    const [lockPeriodInDays, compoundType, amount, reward, prematurePenalty, stakedAt, endedAt] = await _contractStaking.getStakedDetail(address, index)
    return {
        lockPeriodInDays: Number(lockPeriodInDays),
        compoundType: Number(compoundType),
        amount: Number(formatUnits(amount, 8)),
        reward: Number(formatUnits(reward, 8)),
        prematurePenalty: Number(formatUnits(prematurePenalty, 8)),
...
export const getStakingV3Details = async (chainId: number, address: string) => {
    try {
        ...
        const [_userStakedLength, { stakedAmount, stakerPendingReward }] = await Promise.all([_contractStaking.userStakedLength(address), _contractStaking.stakerDetail(address)])
        const _stakedDetails: STAKEV3_DETAIL_ITEM[] = await Promise.all(Array.from({ length: Number(_userStakedLength) }, (_, index) => index).map((idx: number) => getStakingV3Detail(chainId, address, idx, provider)))

        return {
            stakedAmount: Number(formatUnits(stakedAmount, 8)),
            stakerPendingReward: Number(formatUnits(stakerPendingReward, 8)),
...
export const getStakingV3StakedDetails = async (chainId: number, address: string) => {
    try {
        ...
        const { address: STAKING_CONTRACT_ADDRESS, abi: STAKING_ABI } = CONTRACTS[chainId].STAKING_V3
        const _contractStaking = new ethers.Contract(STAKING_CONTRACT_ADDRESS, STAKING_ABI, provider)
        const [{ stakedAmount, stakerPendingReward }, _userStakedLength] = await Promise.all([_contractStaking.stakerDetail(address), _contractStaking.userStakedLength(address)])
...
export const getProjectProgress = async (_project: string, _decimal: number) => {
    ...
    try {
        ...
        price = Number(formatUnits(_price, 6))
    } catch (err) {}
    try {
        const [sale, sold] = await Promise.all([await _contract.sale(), await _contract.sold()])
        return {
            price: price,
            sale: Number(formatUnits(sale, _decimal)),
            sold: Number(formatUnits(sold, _decimal))
        }
    } catch {
        return {
            price: price,
...
export const getRoundDetail = async (_index: number, _decimal: number, address: string, _contract: ethers.Contract, _name: string): Promise<ROUND_DETAIL> => {
    try {
        const [{ start, end, fee_d2, tokenAchieved }, userAllocation, purchasedPerRound] = await Promise.all([_contract.booster(_index), _contract.calcUserAllocation(address, _index), _contract.purchasedPerRound(address, _index)])
        let min = 0
        ...
        return {
            ...
            tokenAchieved: Number(formatUnits(tokenAchieved, _decimal)),
            userAllocation: Number(formatUnits(userAllocation, _decimal)),
            purchasedPerRound: Number(formatUnits(purchasedPerRound, _decimal)),
            min: Number(formatUnits(min, _decimal)),
            max: Number(formatUnits(max, _decimal))
        }
    } catch (err) {
        return {
            name: _name,
...
export const getUserTotalPurchase = async (_project: string, _user: string, _deciaml: number) => {
    try {
        ...
        return Number(formatUnits(_amount, _deciaml))
    } catch (err) {
        console.log(err)
        return 0
    }
...
export const getRoundDetails = async (project: string, decimal: number, address: string, cross: boolean = true) => {
    try {
        ...
        // fetch info
        const [_price, _boosterProgress, _roundsDetails, _whitelist, _userPurchased] = await Promise.all([
            _contract.price(),
        ...
        return {
            ...
            whitelist: Number(formatUnits(_whitelist, decimal)),
            userPurchased: Number(formatUnits(_userPurchased, decimal))
        }
    } catch (err) {
        return {
            price: 0,
...
export const getUSDTBalance = async (address: string, chainId: number) => {
    try {
        ...
        const contract = new ethers.Contract(CONTRACT_ADDRESS, abi, provider)
        const _balance = await contract.balanceOf(address)
        return Number(formatUnits(_balance, decimal))
    } catch (err) {
        console.log(err)
        return 0.0
    }
...
export const getEthAndUsdtBalances = async (address: string): Promise<CHAIN_BALANCE[]> => {
    try {
        const _data = await Promise.all(
            [137, 42161, 56].map(async (_chainId: number) => ({
...
export const getLatestTimestamp = async (provider: JsonRpcProvider) => {
    try {
