...
export const getNativeTokenPrice = async (chainId: number) => {
    ...
    try {
        ...
        const {
            data: { data }
        } = await axios.get(`https://api.coinbase.com/v2/prices/${_chain.symbol}-USD/buy`, { headers })
        return data.amount
    } catch (err) {
        console.log(err)
        return 0
...
export const getChartData = async (chainId: number) => {
    return komAPI(`${process.env.KOM_API_URL}/staking/chart/?chainId=${chainId}`)
}
...
export const getKOMTokenPrice = async () => {
    const { status, result } = await komAPI(`${process.env.KOM_API_URL}/website/statistic`)
    if (status === 'success') {
...
export const getStatistics = async () => {
    return komAPI(`${process.env.KOM_API_URL}/staking/statistic`)
}
/**
...
export const getLeaderBoard = async (chainId: number) => {
    return komAPI(`${process.env.KOM_API_URL}/staking/leaderboard/?chainId=${chainId}`)
}
/**
...
export const getPastStakingDetails = async (chainId: number, address: string) => {
    try {
        const { status, result } = await komAPI(`${process.env.KOM_API_URL}/staking/past/?chainId=${chainId}&address=${address}`)
        if (status === 'success') {
            return result
        } else {
            throw ''
...
export const getPaginationButtons = (total: number, pageNum: number, page: number) => {
    ...
    if (count <= 10) {
        buttons = [page <= 1 ? undefined : { text: '👈 back' }, ...new Array(count).fill('').map((item: number, index: number) => ({ text: String(index + 1) })), page >= count ? undefined : { text: 'next 👉' }]
    } else if (page < 5) {
        buttons = [
