import { getLPStakingDetails } from '@/bot/utils'
    ...
    if (amount === undefined) {
        ...
        if (isNaN(_amount)) {
            ...
            ctx.reply(`\n💬 You entered ${_amount} $LP tokens.\nDo you want to run this transaction? ...👇`, {
                ...
                reply_markup: {
                    force_reply: true,
                    keyboard: [[Markup.button.webApp('✔ O K', `${process.env.MINIAPP_URL}/transactions/staking/lp/stake?chainId=${chainId}&stakingAmount=${_amount}`)], [{ text: '👈 BACK' }]],
                    one_time_keyboard: true,
                    resize_keyboard: true
                }
            })
