...
export const menu = async (ctx: any) => {
    ...
    // get token balances
    const [_balance, { amount, claimableEpoch, index }] = await Promise.all([getLPBalance(address), getLPStakingDetails(address)])
    ...
    await ctx.replyWithPhoto(STAKING_LP_BANNER_IMAGE, {
        caption: msg,
        parse_mode: 'HTML',
        reply_markup: {
            keyboard: [
                [{ text: 'Refresh 💫' }],
                chainId !== 137 ? [{ text: 'Stake 🎨' }, Markup.button.webApp('Switch to Polygon 💦', `${process.env.MINIAPP_URL}?chainId=${chainId}&forChainSelection=true`)] : [{ text: 'Stake 🎨' }],
