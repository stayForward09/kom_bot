    ...
    if (Object.keys(modes).includes(selectedOption)) {
        ...
        if (compoundType === newCompoundType) {
            ...
            ctx.reply(`⚠ You have selected *<b><i>${selectedOption}</i></b>*.\n\nDo you agree to run this transaction? ...👇`, {
                ...
                reply_markup: {
                    force_reply: true,
                    keyboard: [[Markup.button.webApp('✔ O K', `${process.env.MINIAPP_URL}/transactions/staking/v3/change?chainId=${chainId}&compoundMode=${newCompoundType}&index=${index}`)], [{ text: '👈 BACK' }]],
                    one_time_keyboard: true,
                    resize_keyboard: true
                }
            })
