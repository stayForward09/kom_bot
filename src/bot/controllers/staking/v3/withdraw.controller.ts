...
export const textHandler = async (ctx: any) => {
    ...
    const {
        withdraw: { version, index, available, withdrawAmount }
    } = ctx.scene.state
    ...
    if (!withdrawAmount) {
        ...
        if (isNaN(withdrawAmount)) {
            ...
            ctx.reply(`\n✔  You entered ${withdrawAmount} $KOM tokens to withdraw, \n\nDo you agree to run this transaction? ...👇`, {
                ...
                reply_markup: {
                    force_reply: true,
                    keyboard: [[Markup.button.webApp('✔ O K', `${process.env.MINIAPP_URL}/transactions/staking/v3/unstake?chainId=${chainId}&withdrawAmount=${withdrawAmount}&index=${index}`)], [{ text: '👈 BACK' }]],
                    one_time_keyboard: true,
                    resize_keyboard: true
                }
            })
