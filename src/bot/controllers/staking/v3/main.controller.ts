import { getPastStakingDetails, getStakershipDetails, getStakingV3StakedDetails, reduceAmount, formatNumber } from '@/bot/utils'
...
export const menu = async (ctx: any) => {
    ...
    const [
        { nativeBalance, komBalance, komvBalance, komTokenPrice },
        { stakedAmount, stakerPendingReward },
        { originStakership, pendingStakership },
        nativeTokenPrice,
        { nativeBalance: nativeBalance_Pol, komBalance: komBalance_Pol, komvBalance: komvBalance_Pol, komTokenPrice: komTokenPrice_Pol },
    ...
    await ctx.replyWithPhoto(STAKING_V3_BANNER_IMAGE, {
        caption: msg,
        parse_mode: 'HTML',
        reply_markup: {
            keyboard: [
                ...
                [{ text: 'Refresh 🎲' }, Markup.button.webApp(chainId === 137 ? 'Switch to Arbitrum 🎨' : 'Switch to Polygon 🎨', `${process.env.MINIAPP_URL}?chainId=${chainId}&forChainSelection=true`), { text: '👈 Back To Staking Menu' }]
            ],
...
export const stakingV3_ongoing_staking_details = async (ctx: any) => {
    ...
    const { nativeBalance, komBalance, komvBalance, komTokenPrice } = await getTokenBalances(chainId, address)
...
export const staingV3_past_staking_details = async (ctx: any) => {
    ...
    if (stakedDetails.length === 0) {
        return await ctx.reply('😶 You have no past staking details', {
            parse_mode: 'HTML',
            link_preview_options: {
                is_disabled: true
            }
    ...
    stakedDetails.forEach(({ ev_data, tx_hash }: STAKEV3_PAST_DETAIL_ITEM, idx: number) => {
        text +=
            `🎨 <b>${formatUnits(ev_data.amount, 8)} <i>$KOM</i></b>    ( 👁‍🗨 <a href='${_chain.explorer}tx/${tx_hash}'>${tx_hash.substring(0, 15)}...</a> )\n` +
            `<i>- Rewards</i>: <b>${formatUnits(ev_data.reward, 8)}</b><i>$KOM</i>\n` +
            ...
            (ev_data.isPremature ? `<i>- Prematurity Penalty:</i> <b>${formatUnits(ev_data.prematurePenalty, 8)}</b> <i>$KOM</i>\n` : '') +
...
export const chart = async (ctx: any) => {
    ...
    chartPolygon.data.map((item: CHART_DATA_ITEM) => {
        chartPolygonMsg += `- <i>${item.period}</i> = ${formatNumber(formatUnits(BigInt(item.amount), 8))} $KOM\n`
    })
    ...
    charArb.data.map((item: CHART_DATA_ITEM) => {
        chartArbMsg += `- <i>${item.period}</i> = ${formatNumber(formatUnits(BigInt(item.amount), 8))} $KOM\n`
    })
    ...
    await ctx.reply(statisticsMsg + chartPolygonMsg, {
        parse_mode: 'HTML'
    })
    await ctx.reply(chartArbMsg, {
        parse_mode: 'HTML'
    })
}
