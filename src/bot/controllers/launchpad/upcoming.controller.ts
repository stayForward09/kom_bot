import { drawLogoWithBanner, formatNumber, getPaginationButtons, getStakingV3StakedDetails, komAPI } from '@/bot/utils'
...
export const menu = async (ctx: any) => {
    try {
        ...
        // page settings
        await ctx.reply(`⏰ Loading upcoming list ${keyword ? ` with keyword of  *${keyword}*` : ''}...`);
        const projects = await getProjects('upcoming', address, String(keyword))
        ...
        // pagination buttons
        await ctx.reply(
            `⏰ Loading ${page} page of upcoming projects...`,
            {
                reply_markup: {
    ...
    try {
        ...
        if (!project.project) {
            ...
            if (voted) {
                ...
                const { stakedAmount: _totalKOMStaked } = await getStakingV3StakedDetails(137, address);
